...
describe("PolicyPipeline", () => {
  ...
  test("denies when a failed finding meets the policy confidence threshold", async () => {
    const pipeline = new PolicyPipeline({
      evaluator: () => [
        {
          policyId: "no-secrets",
  ...
  test("does not deny when a failed finding is below the policy confidence threshold", async () => {
    const pipeline = new PolicyPipeline({
      evaluator: () => [
        {
          policyId: "no-secrets",
  ...
  test("does not deny from an escalation parent when nested policies pass", async () => {
    const pipeline = new PolicyPipeline({
      evaluator: ({ policies }) => {
        if (policies[0]?.id === "possible-secrets") {
          return [

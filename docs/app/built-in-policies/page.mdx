...
Use `message` to tune user-facing denial copy, `confidence` to require a minimum failed-finding confidence, and `evaluator` when a policy should use a different evaluator from the rest of the pipeline.

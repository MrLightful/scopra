...
`severity` is report-only metadata that describes how serious a finding is, especially when the finding fails. It does not change whether a request is denied.

## Confidence Thresholds

Set `confidence` on a policy when a failed finding should only deny above a minimum confidence score.
Confidence represents how sure the evaluator is; severity represents how bad the finding would be.

...
const refundApprovalPolicy = new Policy({
  ...
  message: "Refunds above the approved threshold need manager review.",
});
```
...
if (!decision.allowed) {
  return decision.violations[0]?.message ?? "This request needs review.";
}
```
...
Allowed decisions have no blocking violations. Denied decisions contain violations with the policy, evaluator finding, and message.
